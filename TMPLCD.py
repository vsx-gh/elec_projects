...
def loop(lcd, firstC, sensor):
    while True:
        prevTemp = firstC 
        temp_C = getTemp(sensor)
        temp_F = convertTempF(temp_C)

        #lcd.clear()
        lcd.message("Lab Temp: \n" + str(temp_C) + " C / " + str(temp_F) + " F")
        time.sleep(5)
        lcd.clear()
...
if __name__ == '__main__':
    try:
        ...
        TMP36_pin = 0
        readadc.initialize()
        myLCD = Adafruit_CharLCD()
        loop(myLCD, inittemp_C, TMP36_pin)
